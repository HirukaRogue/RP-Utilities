...
class PrefixCog(commands.Cog):
    ...
    async def prefix(self, ctx: commands.Context, *, prefix: str) -> None:
        await self.bot.db_loop.run_until_complete(self.bot.database.set_prefix(guild_id=ctx.guild.id, prefix=prefix))
        await ctx.send(f"Prefix set to `{prefix}`!")
    @prefix.command()
    async def reset(self, ctx: commands.Context) -> None:
        await self.bot.db_loop.run_until_complete(self.bot.database.remove_prefix(guild_id=ctx.guild.id))
